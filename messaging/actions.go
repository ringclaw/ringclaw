...
func ensureCoverageHandoffNoteActions(actions []AgentAction, opts ActionContext) []AgentAction {
	if opts.MeshTaskCreator == nil || !opts.OriginIsOwner || opts.SourceTaskID != "" || strings.TrimSpace(opts.InteractiveOriginChatID) != "" || len(actions) == 0 || hasActionType(actions, "NOTE") {
		return actions
	}
	insertAt := -1
