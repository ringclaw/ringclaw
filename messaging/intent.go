...
var intentTriggers = []string{
	...
	"发短信", "短信", "短信给", "短信发给", "发个短信", "发条短信", "发个消息给", "发sms",
	...
	"send sms", "send a text", "text message", "send text to", "sms to",
...
func deterministicIntent(text string) (Intent, bool) {
	...
	if matchesDeterministicSMSIntent(lower) {
		return IntentSMS, true
	}
...
func matchesDeterministicSMSIntent(lower string) bool {
	for _, kw := range []string{
		"发短信", "短信给", "短信发给", "发个短信", "发条短信",
		"send sms", "send a text", "send text to", "text message", "sms to",
	} {
		if strings.Contains(lower, kw) {
			return true
		}
