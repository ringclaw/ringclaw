...
| `chat_ids` | string[] | `[]` | Chat IDs the bot is allowed to receive messages from. The bot's own DM is auto-allowed separately. When empty, unlisted group chats are denied unless `allow_unlisted_group_chats` is explicitly enabled. |
| `allow_unlisted_group_chats` | bool (nullable) | `false` | When `true`, RingClaw keeps `chat_ids` for explicitly listed chats and bot DM routing, but also accepts messages from any non-bot-DM chat the bot is already in. Sender allowlists and `group_mention_only` still apply. |
