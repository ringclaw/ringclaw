...
| `allow_group_mention_authorize` | bool (nullable) | `false` | When `true`, a non-trusted user's `@bot` in an allowed group chat triggers an OOB approval challenge to the owner DM instead of being silently dropped. On approval the requester is added to `chat_user_allow[<chatID>]` (chat-scoped only) and persisted to `config.json`. Requires Private App + resolvable owner DM; otherwise the feature is disabled at startup with an `ERROR` log. See [Security › Authorize-mention OOB flow](../security/sender-allowlist.md#authorize-mention-oob-flow). |
