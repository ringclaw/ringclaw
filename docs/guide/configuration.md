...
| `chat_ids` | string[] | `[]` | Chat IDs the bot is allowed to receive messages from. When empty, RingClaw listens to all chats; this is a high-risk configuration and is usually only appropriate with a strict `source_user_ids` / `chat_user_allow` policy. |
