...
| `api_addr` | string | `127.0.0.1:18010` | `host:port`. Do NOT bind to `0.0.0.0`; see [Security › API & Clients](../security/api-and-clients.md). |
...
| `allow_group_mention_authorize` | bool (nullable) | `true` (since v0.4.1; unset = on) | Controls the OOB approval flow for non-trusted `@bot` in allowed group chats. Unset / `true` → the mention triggers a `/approval` challenge in the owner DM; on approval the requester is added to `chat_user_allow[<chatID>]` (chat-scoped only) and persisted to `config.json`. Set to `false` to keep the legacy silent-drop behavior. Requires Private App + resolvable owner DM; if those are missing the feature is disabled — `ERROR` log when explicitly `true`, `INFO` log when defaulted. After deny / expire, the same `(chat, user)` pair is silenced for 24 hours so a noisy non-trusted user cannot spam the owner DM. See [Security › Authorize-mention OOB flow](../security/sender-allowlist.md#authorize-mention-oob-flow). |
| `chat_user_allow` | map[string][]string | `{}` | Per-chat allowlist layered on top of `source_user_ids`: `{"<chatID>": ["alice@example.com", "3061708020"]}`. Identifiers may be numeric extension IDs, emails, or E.164 phone numbers (resolved to numeric IDs at startup via the Private App directory). Populated automatically by the authorize-mention OOB flow on approval; operators may also pre-seed by hand. Empty / absent chat key = no per-chat exception. Only widens the **sender** allowlist for that chat — does **not** unlock privileged Layer 1 commands (`/cwd`, `/cron`, `/new`, `/reload`, `/full-access`, summarize NL trigger). |
