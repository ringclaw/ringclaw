...
| Authorize-mention OOB flow (`allow_group_mention_authorize`) | Disabled at startup. Logs `ERROR ...` when explicitly opted in (`allow_group_mention_authorize: true`); logs `INFO ...` when defaulted on (v0.5.0+). Either way, non-trusted `@bot` falls back to silent drop. |
