...
`config.json`. It is gated behind a single opt-in field
(`ringcentral.allow_group_mention_authorize: true`) and is **off by
default** — operators who do not opt in see the legacy silent-drop
behavior unchanged.
...
- **Pending dedupe.** A pending challenge for a `(chatID, userID)`
  ...
  prompt-post failure, so the user can retry on the next `@bot`
