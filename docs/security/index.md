...
| `ringcentral.chat_user_allow` | `config.json` (under `ringcentral`) | Per-chat trusted-sender exception. **v0.4.2 force-clears this map at startup** with an ERROR log because listed users gain the bot's full agent capability (FS / terminal / HTTP) until v0.5.0's restricted backend ships. Operators must re-add by hand after re-evaluating. |
