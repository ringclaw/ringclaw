...
| `ringcentral.chat_user_allow` | `config.json` (under `ringcentral`) | Per-chat trusted-sender exception. **v0.4.4+: preserved across restarts.** Listed users run under the v0.4.3 non-owner ceiling (read-only ACP mode + fail-closed deny of `fs/*` / `terminal/*` / `session/request_permission`). Map keys must be numeric RC chat / group IDs — see [Sender Allowlist › How to find a chat ID](./sender-allowlist#how-to-find-a-chat-id). |
