...
- `/mem del` without the trailing `confirm` token never actually clears memory — it only prints a confirmation prompt. This prevents accidental clears in shared scrollbacks.
