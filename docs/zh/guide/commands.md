...
| `/remember [user\|chat\|global] <文本>` | 追加到 persona memory（默认当前 chat）。见 [配置 › persona](./configuration.md#persona)。 |
| `/recall [user\|chat\|global]` | 查看对应 scope 的 memory |
| `/forget [scope]` / `/forget [scope] confirm` | 清空指定 scope memory（需要再发一次带 `confirm` 的命令） |
