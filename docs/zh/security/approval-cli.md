...
| Authorize-mention | `authorize user <userID> in chat <chatID>` | 非授信用户在允许群聊中 `@bot`（v0.5.0 起默认开启；可通过 `allow_group_mention_authorize: false` 关闭） | [发送者白名单](./sender-allowlist#authorize-mention-oob-流程) |
